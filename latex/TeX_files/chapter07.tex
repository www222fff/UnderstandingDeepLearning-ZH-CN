...
f(x, \phi) = \beta_3 + \Omega_3h_3 \tag{7.1}
\end{align} 
...
&f_k = \beta_k + \Omega_k h_k. \quad k \in \{1,2,...,K\} \tag{7.23}
\end{align} 
...
&\frac{\partial \ell_i}{\partial f_{k-1}} = \mathbf{1}[f_{k-1} > 0] \odot \left( \Omega_k^T \frac{\partial \ell_i}{\partial f_k} \right)  \tag{7.24}
\end{align} 
...
&\frac{\partial \ell_i}{\partial \Omega_0} = \frac{\partial \ell_i}{\partial f_0} x_i^T \tag{7.25}
\end{align} 
...
&= \sum_{j=1}^{D_h} \sigma^2 E[h_j^2] = \sigma^2 \sum_{j=1}^{D_h} E[h_j^2], \tag{7.29}
\end{align} 
...
&+ \phi_2a[\psi_{02} + \psi_{12}a[\theta_{01} + \theta_{11}x] + \psi_{22}a[\theta_{02} + \theta_{12}x]] \tag{7.34}
\end{align} 
...
\caption{图 7.4 反向传播过程中的反向传播第一阶段。我们从函数的最末端开始逆向进行，计算损失 l 对于各个中间变量的导数 ∂ li /∂ f• 和 ∂ li /∂ h•。每个导数的计算都是基于前一个导数，并乘以形如 ∂fk/∂hk 或 ∂hk/∂fk−1 的项进行的。}
\end{figure}

\begin{figure}[h!]
\centering
\includegraphics[width=0.7\linewidth]{png/chapter7/Train2BP3.png}
\caption{图 7.5 反向传播过程中的第二阶段。我们最终计算了导数 ∂ li /∂ β• 和 ∂ li /∂ ω•。每个导数的计算都是通过将 ∂li/∂fk 项乘以适当的 ∂fk/∂βk 或 ∂fk/∂ωk 来实现的。}
\end{figure}
